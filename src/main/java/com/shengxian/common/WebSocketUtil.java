...
public class WebSocketUtil {
	...
	private static Log log = LogFactory.getLog(WebSocketUtil.class);
	...
	public void onMessage(String message) throws IOException {
		...
		if (!jsonTo.getString("bid").equals("All")){
			log.info("单方推送");
			sendMessageTo(mes, Integer.valueOf(jsonTo.getString("bid")));
		}else{
			sendMessageAll("给所有人");
			log.info("给所有人");
		}
	...
	public void sendMessageTo(String message, Integer bid) throws IOException {
		// session.getBasicRemote().sendText(message);
		//session.getAsyncRemote().sendText(message);
		for (WebSocketUtil item : clients.values()) {
			if (item.token.equals(bid) )
				log.info("发送成功");
